...
При такой конфигурации ориентировочно **от ~2 000–2 600 ₽/мес** за ВМ (2 vCPU, 2 GB); при уменьшении до 1 vCPU / 1 GB — меньше. Цены меняются, проверяй в [калькуляторе Yandex Compute Cloud](https://cloud.yandex.ru/docs/compute/pricing) или в счёте в консоли.
...
|---|---|---|---|
| Стоимость | 0 ₽ | от ~7 $/мес | ~2 000–2 600 ₽/мес (2 vCPU/2 GB); меньше при 1 vCPU/1 GB |
...
**Как уменьшить стоимость уже после переезда:** см. [YANDEX_COST_REDUCTION.md](YANDEX_COST_REDUCTION.md) — уменьшение ВМ, настройка БД, трафик, гранты.

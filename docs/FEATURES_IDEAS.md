...
- **Чаты:** список, пользовательские папки (до 5), ручная сортировка, создание 1-1 и групп, участники, инвайты по коду, выход, передача владельца, переименование, роли owner/admin.
- **Сообщения:** текст, картинки, видео, файлы, голосовые, редактирование, удаление, ответ/цитирование, пересылка, реакции, закрепление, прочитано, поиск по чату, галерея медиа, превью ссылок, упоминания @username, **экспорт чата в TXT**.
- **Реальное время:** WebSocket (новые сообщения, печатает, онлайн), push-уведомления (FCM), **голосовые звонки 1-на-1** (WebRTC + сигнализация по WS, coturn).
- **Офлайн:** кэш сообщений (Hive), очередь отправки, повтор при ошибке.
- **Профиль:** логин/ник, аватар, смена пароля, удаление аккаунта; EULA при первом входе; экран профиля другого пользователя.
- **Тема:** тёмная и светлая (`Ультрафиолет` / `Аврора`), переключатель в настройках, сохранение выбора.
- **Модерация:** жалоба на сообщение, блокировка пользователя.
- **E2EE (базовый):** шифрование сообщений и медиа в чатах (не полноценные «секретные чаты» Signal-уровня).
- **Desktop:** drag-and-drop файлов в чат (`desktop_drop`).
- **Приватные разделы (по списку в env):** ученики, уроки, балансы, депозиты, отчёты, выгрузка учёта, загрузка банковской выписки, audit отчётов.
...
> **Статус:** ✅ — реализовано в приложении; ⏳ — частично; пусто — не сделано.

### Безопасность и вход

| Статус | Идея | Описание | Сложность |
|--------|------|----------|-----------|
| | **Восстановление пароля** | «Забыли пароль?» → ввод email → письмо с ссылкой/кодом → смена пароля. В миграциях уже есть `recovery_email`, `password_reset_tokens` — можно довести до конца. (Сейчас только сброс пароля админом.) | Средняя |
...
| Статус | Идея | Описание | Сложность |
|--------|------|----------|-----------|
| ⏳ | **Архив чатов** | Скрыть чат из основного списка в «Архив»; при новом сообщении — вернуть в список (как в Telegram). Есть **папки чатов**, но нет отдельного архива с авто-возвратом. | Средняя |
| | **Закреплённые чаты** | Отдельная секция «Закреплённые» вверху списка чатов (сейчас есть только папки и закреплённые **сообщения**). | Низкая |
...
| ✅ | **Видеосообщения** | Запись/выбор видео из галереи или камеры и отправка в чат. | Высокая |
| ⏳ | **Голосовые/видеозвонки** | Звонок 1-на-1 (и при желании группа) через WebRTC; сигнализация по WebSocket. См. раздел ниже. **Голос 1-на-1 ✅**, видеозвонки и групповые — нет. | Высокая |
| | **Стикеры / GIF** | Набор стикеров или выбор GIF (например, GIPHY API) и отправка в чат. | Средняя |
| ✅ | **Экспорт переписки** | Выгрузка чата в TXT (меню чата → «Экспорт чата»). PDF — нет. | Средняя |

### Поиск и навигация

| Статус | Идея | Описание | Сложность |
|--------|------|----------|-----------|
| | **Глобальный поиск** | Один поиск по всем чатам (по тексту, по отправителю, по дате). (Поиск по **текущему** чату ✅.) | Средняя |
...
| Статус | Идея | Описание | Сложность |
|--------|------|----------|-----------|
| ✅ | **Светлая тема** | Переключатель «Ультрафиолет (тёмная) / Аврора (светлая)» в профиле; сохранение выбора. «Как в системе» — нет. | Средняя |
...
| Статус | Идея | Описание | Сложность |
|--------|------|----------|-----------|
| ⏳ | **Свайпы в списке чатов** | Свайп влево: «Прочитать» / «Скрыть»; свайп вправо: «Закрепить» (дополнение к текущему «Удалить»). Сейчас только **свайп «Удалить»**. | Низкая |
...
| ⏳ | **Отображение «был(а) в сети»** | Более заметный last seen в профиле и в списке чатов. Онлайн по WebSocket ✅, текст «был(а) N мин назад» в UI — нет. | Низкая |

### Админ и модерация

| Статус | Идея | Описание | Сложность |
|--------|------|----------|-----------|
| | **Список пользователей для админа** | Экран со списком пользователей (поиск, фильтр), переход в профиль, сброс пароля, бан. (Сброс пароля по email ✅, но без списка.) | Средняя |
| ⏳ | **Роли в группе** | Расширение ролей: не только owner/admin, но и модератор (например, только удаление сообщений). Базовые **owner/admin** ✅. | Средняя |
...
| ⏳ | **Desktop** | Улучшение UX на desktop: горячие клавиши, контекстное меню, drag-and-drop. **Drag-and-drop файлов в чат ✅**; горячие клавиши — нет. | Низкая–средняя |
| | **Crash-репорты** | Интеграция Sentry (или аналога) для сбора ошибок в проде. | Низкая |
| ⏳ | **Проверка обновлений** | При запуске проверка новой версии в store и предложение обновиться. `VersionCheckService` есть, **не подключён к старту приложения**. | Низкая |

### Долгосрочные / сложные

| Статус | Идея | Описание | Сложность |
|--------|------|----------|-----------|
| ⏳ | **Секретные чаты (E2E)** | Сквозное шифрование для выбранных диалогов (Signal Protocol или аналог). Базовый **E2EE для чатов** ✅, но не отдельный режим «секретный чат». | Очень высокая |
...
| | **Резервное копирование** | Экспорт/импорт данных аккаунта (чаты, настройки) в файл или облако. (Экспорт **одного чата** в TXT ✅.) | Высокая |

---

## Голосовые (и видео) звонки: насколько сложно

> **Статус:** голосовые звонки **1-на-1 реализованы** (`VoiceCallService`, `flutter_webrtc`, coturn, push для входящих). Видеозвонки и групповые — ещё нет.
...
4. Свайпы в списке чатов: «Прочитать», «Закрепить». (Сейчас только удаление.)  
5. Описание и аватар группы.  
6. «Был(а) в сети» в профиле и в списке чатов. (Онлайн по WS есть, last seen в UI — нет.)  
7. Crash-репорты (Sentry).  
8. Проверка обновлений при старте приложения. (Сервис есть, не подключён.)
...
2. ~~**Светлая тема**~~ — ✅ реализовано.  
3. **Архив чатов** — стандартная ожидаемая возможность в мессенджерах (папки есть, полноценный архив — нет).  
4. **Глобальный поиск** — сильно упрощает поиск по старым перепискам.  
5. ~~**Экспорт переписки**~~ — ✅ TXT в меню чата.  
...
9. **Подключить проверку обновлений** — `VersionCheckService` готов, осталось вызвать при старте.

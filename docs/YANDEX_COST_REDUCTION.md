...
**Фактические расходы (как пишет Yandex):** ВМ ~**2 600 ₽/мес**, PostgreSQL ~**3 300 ₽/мес** — итого **~5 900 ₽/мес** без учёта трафика и прочего. Ориентиры ниже приведены к этим цифрам.
...
**Эффект:** при текущих ~2 600 ₽/мес за ВМ уменьшение до 1 vCPU и 1 GB RAM часто даёт примерно **30–50% экономии по Compute** (ориентир — минус 800–1 300 ₽/мес). Точные цены: [тарификация Compute Cloud](https://cloud.yandex.ru/docs/compute/pricing).
...
## 7. Ориентир по суммам (под твои фактические счета)

| Компонент | Сейчас (по счёту Yandex) | После оптимизации (оценка) |
|-----------|--------------------------|----------------------------|
| ВМ (2 vCPU, 2 GB, 15 GB) | **~2 600 ₽/мес** | Уменьшить до 1 vCPU, 1 GB, 10 GB: ориентировочно **~1 300–1 800 ₽/мес** (зависит от тарифа и зоны) |
| Managed PostgreSQL | **~3 300 ₽/мес** | Минимальный класс, один хост, 10 GB — уже так; ниже вряд ли без смены тарифа/провайдера |
| Трафик, Object Storage и т.д. | по факту | Обычно десятки рублей; экономить за счёт раздачи файлов из Object Storage, а не через ВМ |
| **Итого** | **~5 900 ₽/мес** | **~4 600–5 200 ₽/мес** при уменьшении ВМ; возможна экономия ещё за счёт гранта 4 000 ₽ |

Цены в Yandex зависят от зоны, типа диска и класса хоста БД. Точные цифры смотри в [калькуляторе Compute](https://cloud.yandex.ru/docs/compute/pricing) и [тарифах Managed PostgreSQL](https://cloud.yandex.ru/docs/managed-postgresql/pricing); в счёте в консоли видно разбивку по сервисам.

---

## 8. Краткий чеклист

- [ ] Уменьшить ВМ до 1 vCPU, 1 GB RAM (и при необходимости диск до 10 GB) — основной способ снизить ~2 600 ₽ за ВМ.
- [ ] Убедиться, что у БД минимальный класс хоста и один хост без реплики (сейчас ~3 300 ₽/мес).
- [ ] Не добавлять реплику БД ради «красной галочки» HA — это сильно увеличит расходы.
- [ ] Исходящий трафик не раздувать (раздача файлов через Object Storage, а не через ВМ).
- [ ] Проверить гранты/промо Yandex Cloud для ИП/ООО (грант 4 000 ₽ может покрыть большую часть счёта).
- [ ] Останавливать ВМ в нерабочее время только если это не продакшен (платится в основном за диск).

...
- **Звук/вибрация при новом сообщении** — настройка в профиле: вкл/выкл звук, вибрация (если ещё не везде).
...
- **Кэш изображений** — сейчас `Image.network` с `cacheWidth`; при росте числа картинок рассмотреть `cached_network_image` с настраиваемым размером кэша и placeholder’ами.
...
- **Image.network с cacheWidth** — превью 500px, полноэкран 1920px; меньше памяти и трафика.
...
1. **print()** — в продакшене не оставлять лишние логи; обёртка в `if (kDebugMode) { ... }` или пакет `logger` с уровнями.
